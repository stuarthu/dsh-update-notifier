...
export function apply(ctx, config = {}) {
  ...
  /** The provider + agent a bubble needs, resolved once per offer. */
  function askTarget(preferredAgent) {
    ...
    try {
      ...
      provider = userQuestions && typeof userQuestions.ask === "function" ? userQuestions : null;
    ...
    return { provider, agent, ready: Boolean(provider && agent && rooted) };
  }
  ...
  async function askUpgrade(outdated, target) {
    ...
    const { provider: userQuestions, agent } = target;
  ...
  /**
   * One check cycle: `{ ok: true, outdated, ready, checked, unreachable }`, or
   * `{ ok: false, code }` for a cycle that did not produce a verdict. The
  ...
  async function runCheck({ force = false, agent = null, signal = null } = {}) {
    ...
    try {
      ...
      if (entries.length > 0 && unreachable === entries.length) {
        return { ok: false, code: "unreachable", checked: entries.length };
      }
      if (outdated.length) {
      ...
      return { ok: true, outdated, ready, checked: entries.length, unreachable };
    } catch (err) {
  ...
  const REASONS = {
    ...
    unreachable: (r) => `the registry could not be reached (${plural(r.checked, "plugin")} unchecked)`,
