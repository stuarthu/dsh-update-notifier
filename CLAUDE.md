...
`dsh-update-notifier` is a single-file DeepSeek Harness (dsh) plugin, published to npm, that hourly — or on demand via its `/check-updates` command — checks the registry for newer versions of the plugins installed in a dsh profile and pops a dsh-rendered approval bubble asking which to upgrade. Approved upgrades are executed by the *agent*, never by this plugin. All logic lives in `lib/index.js` (ESM, Node >= 18 for global `fetch`, zero runtime dependencies; `@deepseek-ai/dsh-llm` is an optional peer dependency provided by the dsh host).
...
- **Check**: `runCheck()` fires `initialDelay` ms after boot, and the `setInterval` is armed at `apply()` — so cycles land every `interval` ms from *boot*, not from the first check. Each fetches `<registry>/<pkg>/latest` per package (AbortController timeout, all failures → retry next cycle). A lookup that could not be answered — a thrown fetch, a timeout, a 5xx, a 429 — returns the `UNREACHABLE` sentinel rather than `null`, which exists so `/check-updates` can count "couldn't ask" apart from "no such package" (404/403).
- **Manual trigger**: `/check-updates` (`ctx.inject(["commands"], …)` → `commands.register()`, the same idiom as dsh's own `permission-presets`) calls `runCheck({ force: true, agent, signal })` and renders the returned result through `describe()` as the command's `CommandResult`.
- **Bubble**: `askTarget()` resolves the (`ctx.get("userQuestions")`, agent) pair — the invoking agent when `/check-updates` supplies one, otherwise the most recently active session's live agent (tracked via `session/event` `user/message`, same as dsh-chrome's page-injector) — and `offerUpgrade()` hands that one resolved target to `askUpgrade()`, which calls `.ask()` with one multi-select question whose options are `pkg current -> latest`. The web provider (`packages/host/apiproxy` in the dsh source) rejects agentless requests and replays pending questions on mux-open, so the bubble survives a refresh.
...
- **Only a *cycle* started by the command forces.** `/check-updates` ignores `announced` *and* `declines` — an explicit ask must never settle as "no updates" because of a remembered "no" — while every timer-started cycle keeps both. (A forced offer that is aborted rather than answered is still held in `pendingOutdated` and re-delivered by the ordinary user-message retry: that is the standing "an abort is not a decline" rule re-delivering the offer you asked for, not the timer forcing anything.)
- **The command settles on the sweep, not on the answer.** `runCheck` does not await `askUpgrade`, so the bubble outlives the command row. Awaiting it would park the handler under `invocation.signal` — the *UI request's* abort — and a refresh or cancel would then kill a bubble that is otherwise designed to survive one.
- **A cancelled row stops the sweep**, both between round trips (`halted()`, which also covers disposal) and inside one — the caller's `signal` is threaded into `latestVersion`, so cancelling costs at most the current fetch rather than `fetchTimeout`. A sweep that ran on would raise a bubble nothing on screen explains.
- **Every un-awaited offer goes through `offerUpgrade()`**, which owns the promise *and* resolves the bubble's target exactly once, returning `ready` so no caller predicts separately what the offer will do. A bare `void askUpgrade(...)` also leaves the region outside that function's own `try` able to produce an unhandled rejection — which by Node's default kills dsh, the one thing this plugin must never do.
- **`runCheck` returns codes, never prose.** It is the timers' function too; `describe()` is the only place a settlement sentence is composed, and a code with no entry in `REASONS` is one the user cannot see (`cancelled` — the host's `withAbort` has already settled that row and discards the handler's value).
- **`ready` reuses the host's admission rule**, not a weaker copy: `ask()` rejects a delegated caller (`DELEGATED_CALLER`), so `askTarget` consults `agents.roots()` before promising "see the question above".
- **A failed lookup must never read as "up to date".** `latestVersion` returns `UNREACHABLE` for a thrown fetch *and* for 5xx/429 (an outage's usual shape), reserving `null` for 404/403; a sweep where every package was unreachable settles as an error, not as "no updates".
- **The command registration's `catch` must sit *inside* the `ctx.inject` callback** — cordis defers it by a microtask, so an outer `catch` cannot see a rejected `register()`. The full reasoning is in the code comment there; keep it there. `ctx.inject` (not `export const inject`) is what keeps a composition with no command registry loading and checking hourly.
